...
**Note: The build step may fail since services the build relies on are no longer available. In this case, you can download a ready build image from the [GitHub Releases](https://github.com/aaronpeikert/reproducible-research/releases/latest) and use `docker load -i workflow-showcase.tar.gz` to load it. Continue with `make all DOCKER=TRUE`**
